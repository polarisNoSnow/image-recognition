 ...
 * 当视频过大的时候，会出现内存溢出的问题，主要原因在于生产者生产能力太强，而消费者跟不上，导致消息队列占用大量内存
 */
class VideoConversion {
    ...
    // 消费队列
    private ConcurrentHashMap<Integer, BufferedImage> mq = new ConcurrentHashMap<Integer, BufferedImage>();
            ...
            for (; ; ) {
                if ((bufferedImage = mq.get(i)) != null) {
                    try {
                        ...
                        mq.put(i,new BufferedImage(1,1,1));
                    } catch (Exception e) {
